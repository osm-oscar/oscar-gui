...
export class ActiveRefinementsComponent implements OnInit {
  ...
  ngOnInit() {
    this.refinementsService.refinements$.subscribe(refinements => {
      this.keyValueRefinements = refinements.filter(
        refinement =>
          refinement.refinementType === RefinementType.KeyValue &&
          refinement.excluding === false
      );
      this.exKeyValueRefinements = refinements.filter(
        refinement =>
          refinement.refinementType === RefinementType.KeyValue &&
          refinement.excluding === true
      );
      this.keyRefinements = refinements.filter(
        refinement =>
          refinement.refinementType === RefinementType.Key &&
          refinement.excluding === false
      );
      this.exKeyRefinements = refinements.filter(
        refinement =>
          refinement.refinementType === RefinementType.Key &&
          refinement.excluding === true
      );
      this.parentRefinements = refinements.filter(
        refinement =>
          refinement.refinementType === RefinementType.Parent &&
          refinement.excluding === false
      );
      this.exParentRefinements = refinements.filter(
        refinement =>
          refinement.refinementType === RefinementType.Parent &&
          refinement.excluding === true
      );
