...
export class WikiServiceService {
  ...
  async getPictureUrls(entity: string): Promise<WikiData> {
    const wikidataResponse = await fetch(
      `https://www.wikidata.org/w/api.php?action=wbgetclaims&entity=${entity}&format=json&origin=*`
    );
    ...
    if (wikidata.claims.P1082)
      wikiData.population = Number.parseInt(
        wikidata.claims.P1082.pop().mainsnak.datavalue.value.amount,
        10
      );
    if (wikidata.claims.P2046)
      wikiData.area = Number.parseFloat(
        wikidata.claims.P2046.pop().mainsnak.datavalue.value.amount
      );
    if (wikidata.claims.P94)
      wikiData.coatOfArmsUrl = this.getPictureUrl(
        wikidata.claims.P94.pop().mainsnak.datavalue.value
      );
    if (wikidata.claims.P373)
      wikiData.name =
        'Region: ' + wikidata.claims.P373.pop().mainsnak.datavalue.value;
